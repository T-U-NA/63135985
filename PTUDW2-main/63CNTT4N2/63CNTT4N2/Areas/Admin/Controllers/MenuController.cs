...
namespace _63CNTT4N2.Areas.Admin.Controllers
{
    public class MenuController : Controller
    {
        ...
        [HttpPost] // lay su kien khi nhan nut them
        public ActionResult Index(FormCollection form) //dua vao form 
        {
            //Them Loai san pham
            if (!string.IsNullOrEmpty(form["ThemCategory"]))// kiem tra co nhap vo dau tick
            {
                ...
                if (!string.IsNullOrEmpty(form["nameCategory"]))
                {
                    ...
                    foreach (var row in listarr)
                    {
                        ...
                        menu.TypeMenu = "category"; //go bang tay
                        menu.Position = form["Position"];//vi tri cua menu, thi ta phai chon vi tri cho no
                        menu.Parentid = 0;
                        menu.Order = 0;
            ...
            //Them Nha cung cap
            if (!string.IsNullOrEmpty(form["ThemSupplier"])) // nut ta bam them thi phai nut them supplier co duoc nhan hay khong
            {
                //kiem tra dau check cua muc con
                ...
                {
                    TempData["message"] = new XMessage("danger", "Chưa chọn nhà cung cấp");// neu chua tick vo nha cung cap
                }
            }
            ...
            {
                //kiem tra dau check cua muc con
                if (!string.IsNullOrEmpty(form["nameCustom"]) && !string.IsNullOrEmpty(form["linkCustom"]))// phia thoa man 2 dong , namecustom va link co du lieu , thi khi nhan nut them 
                {
                    //tao ra menu custom
                    Menus menu = new Menus();
